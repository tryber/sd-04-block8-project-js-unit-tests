...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    const obj = {};
    const menuRet = createMenu(obj);
    assert.strictEqual(menuRet.hasOwnProperty('fetchMenu') && menuRet.fetchMenu() === obj, true);
          //const menuArg = {food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9}};
          //const menuRet = createMenu(menuArg);
          //assert.strictEqual(menuRet.hasOwnProperty('fetchMenu') && menuRet.fetchMenu {food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9}}}, true);
    ...
    // ```
    const keysFetchMenu = Object.keys(createMenu({food: {}, drink: {}}).fetchMenu());
    assert.strictEqual(keysFetchMenu.length === 2 && keysFetchMenu[0] === 'food' && keysFetchMenu[1] === 'drink', true);
    ...
    // ```
    const menuArg = {food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9}};
    const entriesFoodMenuArg = Object.entries(menuArg.food);
    const entriesDrinkMenuArg = Object.entries(menuArg.drink);
    const entriesFoodFetchMenu = Object.entries(createMenu(menuArg).fetchMenu().food);
    ...
    // ```
    assert.strictEqual(Array.isArray(createMenu(menuArg).consumption), true);
    assert.strictEqual(createMenu(menuArg).consumption.length, 0);
    ...
    // ```
    menuRet.order('coxinha');
    assert.deepStrictEqual(menuRet.consumption, ['coxinha']);
    ...
    menuRet.order('agua');
    menuRet.order('sopa');
    menuRet.order('cerveja');
    assert.deepStrictEqual(menuRet.consumption.length, 4);
    ...
    // ```
    menuRet.order('coxinha');
    assert.deepStrictEqual(menuRet.consumption.length, 5);
    ...
    // ```
    const menuRet2 = createMenu({food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9}});
    menuRet2.order('coxinha');
    menuRet2.order('agua');
    menuRet2.order('coxinha');
    menuRet2.order('sopa');
    menuRet2.order('cerveja');
    assert.strictEqual(menuRet2.pay(), 31.35);
