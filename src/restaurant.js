...
// PASSO 1: Crie uma função `createMenu()` que, dado um objeto passado por parâmetro, retorna um objeto com o
// seguinte formato: { fetchMenu: objetoPassadoPorParametro }.
//
...
// definir o objeto que a `createMenu()` define separadamente dela e,
...
const createMenu = (order) => {
  const menu = {
    fetchMenu: order,
    consumption: [],
    order: element => menu.consumption.push(element),
    pay: () => {
      const lista = menu.consumption;
      const food = menu.fetchMenu.food;
      const drink = menu.fetchMenu.drink;
      const tabela = Object.assign({}, food, drink);
      let soma = 0;
