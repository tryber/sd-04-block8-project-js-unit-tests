...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    // const objetoQualquer = createMenu({ dessert: {}});
    // assert.deepStrictEqual(objetoQualquer.fetchMenu, { dessert: {}} )
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    assert.deepStrictEqual(createMenu('água').fetchMenu,'água');
    const obj = { food: { coxinha: 3.9, sopa: 9.9, sashimi: 39.9 },
    drink: { água: 3.9, cerveja: 6.9 }};
    const objRetornado = createMenu(obj);
    assert.deepStrictEqual(objRetornado.fetchMenu, obj );
    ...
    objRetornado.order('coxinha');
    objRetornado.order('água');
    objRetornado.order('sopa');
    objRetornado.order('sashimi');
    assert.deepStrictEqual(objRetornado.consumption, ['coxinha','coxinha', 'água', 'sopa', 'sashimi']);
    objRetornado.order('coxinha');
    objRetornado.order('água');
    objRetornado.order('coxinha');
    assert.deepStrictEqual(objRetornado.consumption, ['coxinha','coxinha', 'água', 'sopa', 'sashimi','coxinha','água','coxinha']);
    assert.deepStrictEqual(objRetornado.pay(), ((3.9 * 6 + 9.9 + 39.9) * 1.1).toPrecision(4));
  });
});
