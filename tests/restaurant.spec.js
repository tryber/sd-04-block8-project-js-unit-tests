...
describe("#createMenu", () => {
	it("tests the function has the correct behaviour", () => {
		...
		const retornoMenu = createMenu({ refri: 6.9, salgado: 3.9 });
		assert.deepStrictEqual(Object.entries(retornoMenu)[0], [
			"fetchMenu",
			{ refri: 6.9, salgado: 3.9 },
		]);
		// ```
		...
		assert.deepStrictEqual(retornoMenu.fetchMenu, { refri: 6.9, salgado: 3.9 });
		// ```
		...
		assert.deepStrictEqual(retornoMenu.order("refri"), retornoMenu.consumption);
		// ```
		...
		retornoMenu2.order("coxinha");
		retornoMenu2.order("agua");
		retornoMenu2.order("sopa");
		retornoMenu2.order("sashimi");
		assert.deepStrictEqual(retornoMenu2.consumption, [
			"coxinha",
			"agua",
			"sopa",
			"sashimi",
		...
		// ```
		assert.strictEqual(retornoMenu2.pay(), 73.04);
		// objetoRetornado.order('coxinha');
		// objetoRetornado.order('agua');
		// objetoRetornado.order('coxinha');
